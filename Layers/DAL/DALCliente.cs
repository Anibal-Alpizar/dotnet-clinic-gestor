...
namespace Clinic_gestor.Layers.DAL
{
    public class DALCliente : IDALCliente
    {
        ...
        public async Task<Cliente> SaveCliente(Cliente pCliente)
        {
            ...
            SqlCommand command = new SqlCommand();
            // Insert
            string sql = @"Insert into Cliente(IdCliente,Nombre,Apellido1,Apellido2,Correo,Sexo,FechaNacimiento,IdProvincia)
                            values (@IdCliente,@Nombre,@Apellido1,@Apellido2,@Correo,@Sexo,@FechaNacimiento,@IdProvincia)";
            try
            {
                ...
                command.Parameters.AddWithValue("@IdProvincia", 1);
                command.CommandText = sql;
                command.CommandType = CommandType.Text;
        ...
        public async Task<Cliente> UpdateCliente(Cliente pCliente)
        {
            ...
            string sql = @"Update Cliente SET IdCliente = @IdCliente, Nombre = @Nombre, Apellido1 = @Apellido1, Apellido2 = @Apellido2, Correo = @Correo ,Sexo = @Sexo, FechaNacimiento = @FechaNacimiento, IdProvincia = @IdProvincia  Where (IdCliente = @IdCliente) ";
            int rows = 0;
            SqlCommand command = new SqlCommand();
            Cliente oCliente = new Cliente();
            try
