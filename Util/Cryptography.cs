...
public class Cryptography
{
    ...
    static public string DecrypthAES(string pCadena)
    {
        ...
        using(Aes encryptor = Aes.Create())
        {
            Rfc2898DeriveBytes pdt = new Rfc2898DeriveBytes(KEY, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 });
            encryptor.Key = pdt.GetBytes(32);
            encryptor.IV = pdt.GetBytes(16);
            using (MemoryStream ms = new MemoryStream())
            {
                using (CryptoStream cs = new CryptoStream(ms, encryptor.CreateDecryptor(), CryptoStreamMode.Write))
                {
